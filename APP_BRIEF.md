...
- **The Node suite passes clean with no secrets and no database.** Verified
  ...
  local PostgreSQL 16): **2102 tests, 2102 pass, 0 fail, 0 skipped**, exit 0.
  Split the way CI splits it: the non-`*Pg` files with no application env at
  all are **1955 pass / 0 skipped**, and the 19 `*Pg` files against a real
  Postgres are **147 pass / 0 skipped**.
