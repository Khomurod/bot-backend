...
- **The Node suite passes clean with no secrets and no database.** Verified
  ...
  local PostgreSQL 16): **2087 tests, 2087 pass, 0 fail, 0 skipped**, exit 0.
  Split the way CI splits it: the non-`*Pg` files with no application env at
  all are **1950 pass / 0 skipped**, and the 18 `*Pg` files against a real
  Postgres are **137 pass / 0 skipped**.
