...
- **The Node suite passes clean with no secrets and no database.** Verified
  ...
  local PostgreSQL 16): **2551 tests, 2551 pass, 0 fail, 0 skipped**, exit 0.
  Split the way CI splits it: the non-`*Pg` files with no application env at
  all are **2322 pass / 0 skipped**, and the 28 `*Pg` files against a real
  Postgres are **229 pass / 0 skipped**.
