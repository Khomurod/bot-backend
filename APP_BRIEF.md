...
- **The Node suite passes clean with no secrets and no database.** Verified
  ...
  local PostgreSQL 16): **2743 tests, 2743 pass, 0 fail, 0 skipped**, exit 0.
  Split the way CI splits it: the non-`*Pg` files with no application env at
  all are **2466 pass / 0 skipped**, and the 35 `*Pg` files against a real
  Postgres are **277 pass / 0 skipped**.
  ...
  panel **152** in 17 files (`npm test --prefix admin`). **So any failure is a real
