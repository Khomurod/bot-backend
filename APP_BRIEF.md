...
- **The Node suite passes clean with no secrets and no database.** Verified
  ...
  local PostgreSQL 16): **3626 tests, 3626 pass, 0 fail, 0 skipped**, exit 0.
  Split the way CI splits it: the non-`*Pg` files with no application env at
  all are **3183 pass / 0 skipped**, and the 51 `*Pg` files against a real
  Postgres are **443 pass / 0 skipped**. The admin suite is **212 pass in 24
  files**.
