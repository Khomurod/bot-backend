...
- **The Node suite passes clean with no secrets and no database.** Verified
  ...
  local PostgreSQL 16): **2912 tests, 2912 pass, 0 fail, 0 skipped**, exit 0.
  Split the way CI splits it: the non-`*Pg` files with no application env at
  all are **2601 pass / 0 skipped**, and the 38 `*Pg` files against a real
  Postgres are **311 pass / 0 skipped**.
  ...
  panel **166** in 19 files (`npm test --prefix admin`). **So any failure is a real
