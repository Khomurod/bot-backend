...
- **The Node suite passes clean with no secrets and no database.** Verified
  ...
  local PostgreSQL 16): **3065 tests, 3065 pass, 0 fail, 0 skipped**, exit 0.
  Split the way CI splits it: the non-`*Pg` files with no application env at
  all are **2731 pass / 0 skipped**, and the 40 `*Pg` files against a real
  Postgres are **334 pass / 0 skipped**. The admin suite is **183 pass in 21
  files**.
