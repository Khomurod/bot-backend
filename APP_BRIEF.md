...
- **The Node suite passes clean with no secrets and no database.** Verified
  ...
  local PostgreSQL 16): **2618 tests, 2618 pass, 0 fail, 0 skipped**, exit 0.
  Split the way CI splits it: the non-`*Pg` files with no application env at
  all are **2363 pass / 0 skipped**, and the 33 `*Pg` files against a real
  Postgres are **255 pass / 0 skipped**.
