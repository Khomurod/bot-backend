...
- **The Node suite passes clean with no secrets and no database.** Verified
  ...
  local PostgreSQL 16): **3803 tests, 3803 pass, 0 fail, 0 skipped**, exit 0.
  Split the way CI splits it: the non-`*Pg` files with no application env at
  all are **3311 pass / 0 skipped**, and the 57 `*Pg` files against a real
  Postgres are **492 pass / 0 skipped**. The admin suite is **221 pass in 25
  files**.
