 ...
 * unit number, first name, last name, and driver type.
 *
 ...
 *   { group_id, group_name, unit_number, first_name, last_name, driver_type, source }
 */
...
async function parseWithAi(list) {
  ...
  for (let i = 0; i < list.length; i += CHUNK) {
    ...
    const prompt = `You normalize trucking driver group names into structured fields.\n`
      ...
      + `- first_name: the driver's first name; null if unclear.\n`
      + `- last_name: the driver's last/family name (may be empty); null if none.\n`
