/**
 * Choosing WHICH load context a dispatch question is answered from.
 *
 * This suite covers the resolution order and its caching: destination
 * inference, the signature that decides whether a cached context is still
 * valid, which pinned candidate wins, and the fallback chain when no pin
 * exists. The AI extraction that turns chosen text INTO a context is a
 * separate concern and lives in dispatchPinnedContextAi.test.js.
 *
 * The fallback ORDER is the point of the later cases: stored recent loads beat
 * a pinned message when the stored one is complete, and when everything fails
 * the caller gets one canonical message rather than a provider error.
 */
