 ...
 *
 * `lib/security/facebookCrypto.js` stays this repository's standard secret
 * storage. This second one exists for exactly one value — the Samsara API key —
 * because the separate `samsara-integration` service has to open it too and
 * holds none of this app's secrets. See the module header for what that costs.
 *
 * This file is mirrored, deliberately, by
 * samsara-integration/tests/samsaraSharedIntegrationCrypto.test.js — the SAME
 * fixed vector, the same assertions. If a change to one repository's derivation
 * does not reach the other, one of the two suites fails here instead of the
 * poller silently losing its Samsara key in production.
 */
const { test } = require('node:test');
const assert = require('node:assert/strict');

const crypto = require('../lib/security/sharedIntegrationCrypto');
