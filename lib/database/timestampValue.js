...
/**
 * The only two written shapes this accepts, each with its day components.
 *
 * NOT a filter over `Date.parse` — a REPLACEMENT for trusting it. V8 accepts
 * `Feb 30, 2026`, `02/30/2026`, `2026/02/30` and `30 Feb 2026` and silently
 * answers 2 March for all of them, so a guarantee that nonexistent days are
 * refused is only as wide as the shapes it actually checks. Narrowing to the
 * shapes this application receives from machines makes it true everywhere:
 *
 *   ISO-8601        what every API here sends
 *   M/D/YYYY        what Datatruck writes in a `pickup_time`
 *
 * Anything else is refused rather than guessed at. A null is visible in a
 * column and in a report; a wrong date is not.
 */
const SUPPORTED_SHAPES = [
  { pattern: /^(\d{4})-(\d{2})-(\d{2})(?:[T ]|$)/, order: ['y', 'm', 'd'] },
  { pattern: /^(\d{1,2})\/(\d{1,2})\/(\d{4})(?:[T ]|$)/, order: ['m', 'd', 'y'] },
];
...
/**
 * Does the text use a shape we accept, and does it name a day that exists?
 *
 * Checked by ARITHMETIC, not by re-reading the parsed instant. A round trip has
 * to pick a timezone frame to compare in, and the frame depends on the string —
 * a bare date is UTC, a naive time is local, an explicit offset is neither — so
 * it rejected perfectly good timestamps that carried a zone. Whether 30
 * February exists is not a question about timezones.
 */
function namesARealDay(text) {
  for (const { pattern, order } of SUPPORTED_SHAPES) {
    const found = pattern.exec(text);
    if (!found) continue;
    const parts = {};
    order.forEach((key, i) => { parts[key] = Number(found[i + 1]); });
    if (parts.m < 1 || parts.m > 12) return false;
    return parts.d >= 1 && parts.d <= daysInMonth(parts.y, parts.m);
  }
  return false;
}
