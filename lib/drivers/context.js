...
function findContradictions(context, { now = null, activeWithinMinutes = 720 } = {}) {
  ...
  const out = [];
  const { homeTime, loads, fuel, safety, retention, board } = context || {};
  ...
  if (retention?.known && retention.goneQuiet && activity.length) {
    out.push({
      ...
      // NO ONSET DATE, deliberately. `driver_retention_assessments` holds one
      // upserted row per driver for their whole life, so neither of its
      // timestamps is when the quiet started: `first_seen_at` is when the
      // driver was first assessed at all, `last_seen_at` moves every sweep.
      // Presenting either as "quiet since" would be a fabricated fact, and the
      // contradiction does not need one — the point is the DISAGREEMENT, and
      // the activity beside it is dated.
      evidence: {
        activity,
        assessmentSince: retention.assessmentSince || null,
      },
    });
  }

  // ── the board and Wenze disagree about where the driver is ───────────────
  //
  // THE BOARD IS THE AUTHORITY ON TODAY'S ASSIGNMENT AND STILL DOES NOT WIN
  // HERE. It is authoritative about which truck somebody is dispatched in, not
  // about whether they are home — that is Home Time's, built from what the
  // driver said and what the truck did. So a disagreement is reported and
  // neither side is corrected.
  //
  // Both branches require the board to have an OPINION (`says` is 'home' or
  // 'working'). A driver at REST or a truck in SHOP is neither, and folding
  // those into "not home, therefore working" is how a driver legitimately
  // resting gets reported as contradicting their own home-time record.
  if (board?.known && homeTime?.known) {
    if (board.says === 'home' && homeTime.state === 'road') {
      out.push({
        kind: 'board_home_while_road',
        summary: `The dispatcher board has them ${board.statusRaw || 'at home'}, `
          + `and Home Time has them on the road since ${homeTime.stateSince || 'an unrecorded date'}`,
        sides: ['board', 'home_time'],
        evidence: {
          boardStatus: board.status, truck: board.truck,
          stateSince: homeTime.stateSince, boardSeenAt: board.lastSeenAt,
        },
      });
    }
    if (board.says === 'working' && homeTime.state === 'home') {
      out.push({
        kind: 'wenze_home_while_board_working',
        summary: `Home Time has them at home since ${homeTime.stateSince || 'an unrecorded date'}, `
          + `and the dispatcher board has them ${board.statusRaw || 'working'}`,
        sides: ['home_time', 'board'],
        evidence: {
          boardStatus: board.status, truck: board.truck,
          stateSince: homeTime.stateSince, boardSeenAt: board.lastSeenAt,
        },
  ...
  // ── two trucks at once ───────────────────────────────────────────────────
  //
  // NOT REPRESENTABLE TODAY, AND KEPT ANYWAY. Migration 0015 created
  // `uniq_driver_units_open_person ON driver_units (person_id) WHERE ended_at
  // IS NULL`, so the database refuses a second open unit and this branch cannot
  // fire against real rows. It stays because it costs nothing — the list is
  // already in hand — and because it is the only thing that would notice if
  // that index were ever dropped or failed to apply. It deliberately does NOT
  // get a query in `listContradictionCandidates`: a fleet-wide scan every
  // fifteen minutes for a state the schema forbids is a cost with no answer.
