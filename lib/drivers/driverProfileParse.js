 ...
 *
 * DELIBERATELY PERMISSIVE, and not the strict `lib/drivers/fleetType.js` reader.
 * The Dispatcher Board always parenthesises its labels; a Telegram title does
 * not have to, and `COMPANY DRIVERS` written without brackets is a company
 * driver's chat. Requiring the strict form here would silently drop such a group
 * from every company-driver broadcast — see
 * `services/broadcastTargetService.js`, which leans on exactly this leniency.
 *
 * `lease` is here because without it a `(LEASE DRIVERS)` group was created as an
 * `owner`, and once fleet type became authoritative that stored `owner` started
 * recording a lease driver's truck as an owner-operator one — inventing
 * conflicts with a real owner operator of the same number.
 */
function inferDriverType(groupName) {
  const text = String(groupName || '');
  if (/company\s+drivers?/i.test(text)) return 'company_driver';
  if (/lease\s+drivers?/i.test(text)) return 'lease';
  return 'owner';
}
