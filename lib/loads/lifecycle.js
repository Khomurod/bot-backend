...
function derivePhase({ nowIso, load = null, position = null, remembered = {}, options = {} } = {}) {
  ...
  //
  // AND A BOARD RUNNING AHEAD IS NOT ENOUGH EITHER. That was the first rule
  // here and production answered it: 75 of 235 loads came back "conflicted",
  // a third of the fleet, because "ahead of the phase we derived" catches two
  // entirely innocent situations —
  //
  //   A TRUCK PARKED MID-TRIP. Board says loaded; the truck is at a rest stop
  //   nowhere near either end; with no remembered arrival the ladder above
  //   lands on `assigned`. That produced
  //   "the truck has not left the shipper" about a truck 300 miles from it.
  //
  //   A LOAD WE STARTED WATCHING LATE. Board says delivered; the truck has
  //   already moved on. Nothing here saw the arrival because this watcher did
  //   not exist when it happened, and absence of memory is not evidence of
  //   anything. That produced "the truck is still at the receiver" about a
  //   truck driving away from it.
  //
  // So a conflict now requires POSITIVE EVIDENCE that contradicts the board —
  // the truck seen somewhere the board's claim cannot be true from — and each
  // message says the thing that was actually observed. Everything else where
  // the board is ahead is recorded as a SIGNAL, which keeps the confidence
  // honest without making it somebody's question.
  //
  ...
  if (board && board !== phase && PHASE_ORDER.indexOf(board) > PHASE_ORDER.indexOf(phase)) {
    if (board === PHASES.DELIVERED && atPickup) {
      // The strongest one in the set: the board has recorded a completed
      // delivery for a truck standing at the shipper.
      conflicts.push('board_says_delivered_but_the_truck_is_at_the_shipper');
    } else if (board === PHASES.DELIVERED && sawDelivery === false && remembered.phase) {
      // Only with memory. We have been watching this load, never saw it reach
      // the receiver, and the board says it is done.
      conflicts.push('board_says_delivered_but_this_load_was_never_seen_at_the_receiver');
    } else if (board === PHASES.IN_TRANSIT && atPickup) {
      conflicts.push('board_says_loaded_but_the_truck_is_still_at_the_shipper');
    } else {
      // Ahead, but nothing observed contradicts it. Usually a load that was
      // already running when this watcher first saw it.
      signals.push('board_ahead_of_what_has_been_observed');
    }
  }

  let confidence = 'high';
  if (conflicts.length) confidence = 'medium';
  else if (signals.includes('board_ahead_of_what_has_been_observed')) confidence = 'medium';
