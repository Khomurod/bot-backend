...
// THE ONE OWNER OF "CRITICAL", imported rather than copied. `lib/fuel/risk.js`
// decides what an urgent tank is and this has to agree with it; a second copy
// of the number is a second thing to forget when the business changes it.
// lib/ is the bottom layer and risk.js imports only lib/geo, so there is no
// cycle. This module already reasons in fuel terms (range, miles to the stop),
// so knowing the threshold is consistent rather than new coupling.
const { DEFAULTS: FUEL } = require('../fuel/risk');
...
function priorityFor({ severity = 'info', facts = {} } = {}) {
  ...
  // A CRITICAL TANK IS URGENT ON ITS OWN, with no stop to measure against.
  //
  // This was missed: the only fuel facts were `rangeMiles` and
  // `milesToStation`, so a truck at 6% with no assigned stop produced no facts
  // at all, landed at `whenever`, and could be HELD for an hour behind three
  // other notices about the same driver. The percentage is an established
  // number like any other, and it is the one that says "urgent" when there is
  // nothing else to go on.
  if (Number.isFinite(facts.fuelPercent) && facts.fuelPercent <= FUEL.criticalFuelPercent) {
    level = LEVELS.NOW;
    reasons.push(`the tank is at ${Math.round(facts.fuelPercent)}%`);
  }
  ...
  if (Number.isFinite(facts.rangeMiles) && Number.isFinite(facts.milesToStation)) {
    ...
    if (spare <= 0) {
      ...
      // RAISED, NEVER LOWERED. A plain assignment here would drop a `now`
      // already reached by the critical-tank rule back to `today` — the exact
      // shape of bug this whole section exists to prevent.
...
/**
 * What counts as "the same subject" when deciding whether somebody has heard
 * enough already.
 *
 * A PERSON FIRST, because the flood this prevents is about a human: a fuel
 * risk, a load contradiction and a retention signal about one driver, each
 * correctly deduplicated against itself, arriving together and reading as three
 * problems. Grouping by category would defeat that entirely — those three ARE
 * three categories.
 *
 * A group is the fallback because most of this application still identifies a
 * driver by their chat, and the subject pair is the last resort so a notice
 * about a provider or a worker still groups with its own kind rather than with
 * everything else that has no person.
 *
 * Returns null when there is nothing to group by, and `shouldSuppress` then
 * holds nothing — an ungroupable notice is a one-off, and a one-off is never
 * the flood.
 */
function suppressionKeyFor({
  personId = null, groupId = null, subjectType = null, subjectId = null,
} = {}) {
  if (personId != null && personId !== '') return `person:${personId}`;
  if (groupId != null && groupId !== '') return `group:${groupId}`;
  if (subjectType && subjectId != null && subjectId !== '') {
...
module.exports = {
  LEVELS, ORDER, CEILING, priorityFor, shouldSuppress, suppressionKeyFor,
};
