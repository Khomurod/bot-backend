...
/**
 * Is this row a person's deliberate decision, or merely an old one?
 *
 * THE DISTINCTION THAT WAS MISSING. Migration 0058 stamped every row that
 * already existed as `manual`, on the reasoning that somebody had typed it. But
 * those rows were typed BEFORE the Board could place anybody — they are not
 * decisions to overrule the Board, they are simply the old way of doing it. As
 * `manual` they were permanently exempt from every rebuild, so the fleet's
 * whole existing roster could never become automatic and the feature would have
 * looked switched off for ever.
 *
 * An override is a person acting AGAINST the Board with the new system in
 * place, and that act leaves a mark: `markManualOverride` records who and when.
 * So the timestamp is the test, not the word. No timestamp, no override —
 * whatever the source column says.
 */
function isHumanOverride(row) {
  return Boolean(row)
    && row.assignmentSource === 'manual'
    && row.manualOverrideAt != null;
}
...
module.exports = { OUTCOME, REVIEW_REASON, isHumanOverride, personKeyOf, planRoster };
