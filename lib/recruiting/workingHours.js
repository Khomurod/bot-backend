...
function windowCovers(win, local) {
  return coversAt(win, local.weekday, local.hour * 60 + local.minute);
}

/**
 * The same question without a clock: does this window cover `minutes` past
 * midnight on ISO `weekday` (1 = Monday)?
 *
 * THE ONLY IMPLEMENTATION of the coverage rules, so that sweeping a whole week
 * to work out when Wenze can actually speak cannot drift from what the live
 * path decides at the moment a candidate texts. Two copies of this logic is
 * exactly the class of bug this repository keeps finding; `windowCovers` above
 * is now a two-line adapter for callers holding a Luxon instant.
 */
function coversAt(win, weekday, minutes) {
  const today = weekday;
