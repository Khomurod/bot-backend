...
function composeWeeklyFinanceReport(totals, { periodStart, periodEnd }) {
  ...
  if (codes === 0) {
    ...
    // THE HEADLINE IS WHAT IS STILL LIVE, not what was ever posted. A voided
    // code happened and keeps its line below; adding its amount here would
    // report money the company is not out. The two figures are stated
    // separately so neither has to be inferred from the other.
    const voided = Number(t.voidedCount) || 0;
    const replaced = Number(t.replacedCount) || 0;
    const hasLifecycle = t.activeAmount !== undefined && t.activeAmount !== null;
    const headline = hasLifecycle ? t.activeAmount : t.amountTotal;

    lines.push(`<b>${plural(codes, 'code', 'codes')} issued</b>, `
      + `<b>${escapeHtml(money(headline))}</b> still active.`);
    if (voided > 0) {
      lines.push(
        `• ${plural(voided, 'code was', 'codes were')} voided, `
        + `${escapeHtml(money(t.voidedAmount))} — kept in the history and out of that total.`,
      );
    }
    if (replaced > 0) {
      lines.push(`• ${plural(replaced, 'code was', 'codes were')} replaced by another.`);
    }
    if (Number(t.duplicatePostings) > 0) {
      const n = Number(t.duplicatePostings);
      lines.push(
        `• ${plural(n, 'code was', 'codes were')} posted again after already being recorded. `
        + `The repeat is not counted a second time — this says the code was POSTED twice, `
        + 'not that anybody was paid twice.',
      );
    }
  ...
  const unread = (Number(t.messagesNeedingAttention) || 0) + (Number(t.codesNeedingReview) || 0);
