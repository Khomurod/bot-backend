...
const { FIELD, labelToField } = require('./labels');
const { scanCodes } = require('./values');
...
/**
 * The code-shaped digit runs a message NAMES — never the ones it labels as
 * something else.
 *
 * THIS IS THE SAME BUG AS THE ONE THE LABEL PARSER EXISTS FOR, one layer up. A
 * void or a replacement that quotes the production format carries
 * `Report Reference: 165373918` beside the code, and a bare scan of the whole
 * message offers that reference as a candidate money code. The void then names
 * "two codes" and refuses; the replacement names a code nobody holds and
 * refuses. In both cases the reference was never a code, exactly as it was
 * never a code when the parser read the issuing message.
 *
 * So the same rule applies here: a value a label claimed for something else is
 * not a candidate. Unlabelled text is scanned, money-code labels are scanned,
 * and references, amounts, recipients and notes are left alone.
 */
function namedCodesIn(text) {
  const { fields, unlabelledText } = extractFields(text);
  const out = [...scanCodes(unlabelledText)];
  for (const value of valuesFor(fields, FIELD.MONEY_CODE)) {
    for (const code of scanCodes(value)) out.push(code);
  }
  return [...new Set(out)];
}
...
module.exports = { MAX_LABEL_WORDS, splitLine, extractFields, valuesFor, namedCodesIn };
