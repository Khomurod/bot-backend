...
const { namedCodesIn } = require('../moneycode/fields');
...
function classifyVoidLanguage(text) {
  ...
  // NOT a bare scan: a void that quotes the production format carries a
  // Report Reference beside the code, and offering that as a second candidate
  // made the void refuse a target it could see perfectly well.
  const codes = namedCodesIn(source);
