 ...
 *
 * `security/facebookCrypto.js` is the repository's standard secret storage, and
 * it stays that for everything this app reads alone. It cannot be used for the
 * Samsara API key, because the OTHER reader is the separate
 * `samsara-integration` Render service, which holds no
 * FACEBOOK_TOKEN_ENCRYPTION_KEY and no JWT_SECRET — only DATABASE_URL and the
 * Telegram tokens. Handing it either of those would be exactly the "add a new
 * Render environment variable" the operator asked to avoid, and would put this
 * app's session-signing secret on a second service to boot.
 *
 ...
 *
 * Mirrored, deliberately and with the same comments, in
 * samsara-integration/src/sharedIntegrationCrypto.js. The two must agree
 * byte-for-byte on derivation; there is a cross-repo test on each side pinning
 * the envelope against a fixed vector.
 */
