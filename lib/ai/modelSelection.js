...
function reconcileChain(currentChain, discovered, { providerKey = null, freeOnly = false } = {}) {
  ...
  const candidates = proposed.filter((m) => !kept.includes(m));
  const chain = kept.length ? [...kept, ...candidates].slice(0, MAX_CHAIN) : proposed;
  // Only what is actually IN the chain counts as added. Four survivors leave one
  // slot; claiming three newcomers were "added" would tell an operator Wenze
  // switched to models it never configured.
  const added = chain.filter((m) => !kept.includes(m));
