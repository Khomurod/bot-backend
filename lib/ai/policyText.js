...
const WATCHED_TOPICS = [
  ...
  },
  {
    ...
    patterns: [/free\s+(tier|plan|quota|usage|access)/i, /rate\s+limits?/i, /requests?\s+per\s+(day|minute)/i,
      /no\s+cost/i, /trial/i, /free\s+tier\s+(is|will be|has been)\s+(discontinued|removed|retired|ending)/i],
  },
  {
    key: 'pricing',
    label: 'Pricing and billing',
    patterns: [/pric(e|es|ing)/i, /billed|billing/i, /per\s+(token|request|1[km]\s+tokens)/i,
      /paid\s+(service|plan|tier)/i, /\$\s?\d/i, /credits?\s+(expire|required)/i],
  },
  {
    key: 'authentication',
    label: 'How Wenze must authenticate',
    patterns: [/api\s+keys?\s+(will|must|are)\s+/i, /authenticat(e|ion|ing)/i, /oauth/i,
      /bearer\s+token/i, /service\s+account/i, /credentials?\s+(will|must)/i],
  },
  {
    key: 'api_change',
    label: 'API behaviour or version changes',
    patterns: [/breaking\s+change/i, /api\s+version\s+v?\d+\s+(will|is)\s+(be\s+)?(removed|retired|deprecated)/i,
      /migrat(e|ion)\s+to\s+v?\d/i, /endpoint\s+(will|has)\s+(be\s+)?(removed|changed|renamed)/i,
      /(response|request)\s+format\s+(will|has)\s+change/i],
  },
  {
    key: 'retention',
