...
const CAPABILITIES = Object.freeze([
  ...
  {
    key: 'finance_message_reading',
    label: 'Say what a finance message MEANS',
    what: 'Reads a finance-group message the rules could not settle and says what KIND it is — '
      + 'issuing a code, reporting a void, asking for one, or ordinary conversation. It reports '
      + 'meaning only; every digit it returns is checked against the message itself.',
    group: 'Finance',
    changesState: true,
    stateNote: 'Proposes a READING, never a state. The deterministic rules still decide which '
      + 'code a void refers to, and lib/finance/aiReading.js refuses any code, amount or '
      + 'reference that does not appear verbatim in the captured text — so a model can move a '
      + 'message off the unclear pile but can never author a number.',
    sendsRawText: true,
    rawTextNote: 'The message text is sent, fenced as untrusted data. That text is payment '
      + 'information.',
    fallback: 'The message keeps whatever the rules made of it — unparsed, ambiguous or needing '
      + 'a person. Nothing is recorded as read that was not read.',
  },
