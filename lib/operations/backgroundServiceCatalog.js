...
const CATALOG = Object.freeze([
  ...
  { key: 'retention_chat_signals', label: 'hearing what drivers say', group: 'integration', expectedIntervalSeconds: 3600, critical: false, configurable: true },
  { key: 'recruiting_after_hours', label: 'answering candidates after hours', group: 'integration', expectedIntervalSeconds: 3600, critical: false, configurable: true },
  { key: 'leads_bot', label: 'the leads bot', group: 'integration', expectedIntervalSeconds: 3600, critical: false, configurable: true },
  // The authority on who is in which truck TODAY. Configurable because it
  // reads an external spreadsheet nobody has to connect; critical because
  // once it IS connected, every assignment question downstream is answered
  // from a snapshot this worker is the only writer of, and a stale snapshot
  // looks exactly like a fresh one.
  { key: 'dispatch_board_poll', label: 'the Dispatcher Board feed', group: 'integration', expectedIntervalSeconds: 3600, critical: true, configurable: true },

  // Reads the attachments the finance group posts, ONE AT A TIME. Configurable
  // because nothing runs until the Finance Monitor is switched on and document
  // capture with it; not critical because a document waiting to be read is a
  // queue backing up, not a driver going unheard.
  // The weekly money-code report. Nine days rather than seven: a weekly job
  // called stale at day eight would be red every Sunday evening for the whole
  // of its normal life.
  { key: 'finance_weekly_report', label: 'the weekly money-code report', group: 'routine', expectedIntervalSeconds: 86400 * 9, critical: false, configurable: true },
  { key: 'finance_document_reader', label: 'reading finance documents', group: 'engine', expectedIntervalSeconds: 1800, critical: false, configurable: true },
  // Re-reads captured messages an older parser version misunderstood. Rides
  // the scheduler's hourly tick, so two hours is the honest staleness bound;
  // configurable because it has nothing to do until the Finance Monitor is on.
  { key: 'finance_reparse', label: 're-reading finance messages after a parser change', group: 'engine', expectedIntervalSeconds: 7200, critical: false, configurable: true },
  ...
  // NOT A WORKER. The one piece of configuration without which every feature in
  // this application runs, finds real things, and says nothing — which is the
  // silence the whole project started from. It sits in the roster so that
  // "nobody has told Wenze where to send anything" appears in the same list an
  // operator reads for everything else, rather than only on a settings page
  // they have no reason to open.
  { key: 'notification_destination', label: 'where Wenze sends its alerts', group: 'integration', expectedIntervalSeconds: 1800, critical: true, configurable: true },
  ...
  // Rides the scheduler's hourly tick. Critical because the source track record
  // — and the reliability floor built on it — is empty until this runs.
  { key: 'decision_verification', label: 'grading what was decided', group: 'engine', expectedIntervalSeconds: 3600, critical: true },
  // NOT critical, deliberately. It reports disagreements for a person to
  // settle; it corrects nothing and nothing downstream waits on it, so a pass
  // that stops is worth a line on the Operations screen and not an alarm.
  { key: 'contradiction_pass', label: 'features disagreeing about one driver', group: 'engine', expectedIntervalSeconds: 900, critical: false },
  // Asking the owner about what Wenze could fix but has not been told it may.
  // NOT critical: the findings are all still on Needs Attention whether or not
  // anybody is asked, so a stopped ask pass is a channel gone quiet rather than
  // work going undone — and saying it is critical would put the fleet's health
  // light on red for a feature the owner may deliberately switch off.
  { key: 'control_ask_pass', label: 'asking the owner about open findings', group: 'engine', expectedIntervalSeconds: 900, critical: false, configurable: true },
  ...
  // Reads the Dispatcher Board for the other half of the same question: when a
  // driver GOES home. Slower than the return watch because it costs one query
  // over a snapshot somebody else's poller already fetched.
  { key: 'home_time_board_presence', label: 'Home Time home-in/home-out from the dispatcher board', group: 'engine', expectedIntervalSeconds: 1800, critical: true },
  ...
  // THE ONLY DEFENCE AGAINST UNBOUNDED GROWTH, and for the life of the feature
  // it was unobservable: its failures reached `console.error` and nothing else,
  // so every prune could fail for months with nothing on /api/health to say so.
  // Two hours rather than one: the tick is hourly and a prune called stale at
  // sixty-one minutes would flicker red for a scheduler running perfectly.
  { key: 'data_retention', label: 'pruning the tables that grow', group: 'engine', expectedIntervalSeconds: 7200, critical: false },
