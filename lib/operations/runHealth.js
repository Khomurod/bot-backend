 ...
 *   blocked: boolean, silentForMs: number|null, consecutiveFailures: number}}
 */
function classifyRun(row, context = {}) {
  ...
  const verdict = (state, reason, extra = {}) => ({
    ...
    // WAITING ON A PERSON AND FAILING BOTH READ AS `needs_human_attention`,
    // and only one of them is a fault. Callers that merely need "should
    // somebody look?" read `actionable` and see no difference; callers that
    // report a component as WORKING OR NOT must not paint a switch nobody has
    // flipped as an outage, and this is the flag that lets them tell.
  ...
  if (row.lastStatus === 'error') {
    ...
    // An `other` still points somewhere rather than trailing off. The category
    // could not name the cause, and the message that can is one click away on
    // an authenticated screen — saying so is more use than saying nothing.
    const kind = classifyErrorKind(row.lastError);
    const because = kind
      ? ` — ${kind === 'other'
        ? 'the message is on Operations → What is running'
