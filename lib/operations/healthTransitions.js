...
const FAILED = 'failed';
/**
 * Waiting on a person to switch it on — which is NOT the same as broken.
 *
 * `runHealth.js` has always said so: "painting it red is how a real outage gets
 * lost among things that were never switched on." This module was where that
 * intent was being lost. A blocked component is recorded as blocked, never
 * counted among the failures, and never announced — there is nothing to
 * announce, because nothing broke and nothing will recover.
 */
 ...
 * @param {{ok: boolean, blocked?: boolean, detail?: string|null}} observation
...
function observe(previous, observation, options = {}) {
  ...
  const blocked = observation?.blocked === true;
  const ok = !blocked && observation?.ok === true;
  const detail = observation?.detail ? String(observation.detail).slice(0, 300) : null;

  // A SWITCH NOBODY HAS FLIPPED SHORT-CIRCUITS EVERY RULE BELOW. It is not a
  // failure, so it starts no failure count and trips no flap window; and it is
  // not a recovery, so it never says "working again" about something that was
  // never working. If a failure HAD been announced before somebody switched the
  // feature off, the record of that is cleared silently — there is no honest
  // recovery message for "you turned it off".
  if (blocked) {
    return {
      state: {
        ...prev,
        component,
        status: BLOCKED,
        since: prev.status === BLOCKED ? prev.since : nowIso,
        consecutiveFailures: 0,
        consecutiveOk: 0,
        announcedStatus: null,
        lastError: detail,
