...
const { proposeConfidenceFloor } = require('./thresholdProposal');
...
const DEFAULTS = {
  ...
  minRefusals: 3,
  /** Times the owner has answered the same way before it is worth remarking on. */
  minOwnerAnswers: 3,
  /** How far back an owner's answers count. */
  answerWindowDays: 30,
...
/**
 * Checks the owner keeps answering the same way.
 *
 * THE SIGNAL IS DIFFERENT FROM EVERY OTHER ONE IN THIS FILE, and better. A
 * revert is a person undoing something Wenze did; this is a person saying,
 * repeatedly and in words, what they want — the closest thing to a stated
 * business rule the application ever receives. Three "no"s about the same check
 * is the owner telling you it is asking the wrong question.
 *
 * AND IT IS COUNTED ACROSS SUBJECTS, NOT WITHIN ONE. One driver answered three
 * times is one situation that keeps changing; three different drivers answered
 * the same way is a pattern about the CHECK. The second is the only one worth
 * remarking on, so a group needs answers about at least `minOwnerAnswers`
 * distinct subjects.
 */
function groupOwnerAnswers(memories, { nowMs, answerWindowDays, minOwnerAnswers }) {
  const byCheck = new Map();
  for (const m of memories || []) {
    if (m?.revokedAt) continue;
    if (!withinWindow(m?.createdAt, nowMs, answerWindowDays)) continue;
    const key = `${m.checkKey}|${m.answerAction}`;
    if (!byCheck.has(key)) byCheck.set(key, []);
    byCheck.get(key).push(m);
  }
  const groups = [];
  for (const [key, rows] of byCheck) {
    const subjects = [...new Set(rows.map((r) => `${r.subjectType}:${r.subjectId}`))];
    if (subjects.length < minOwnerAnswers) continue;
    const [checkKey, answerAction] = key.split('|');
    groups.push({
      checkKey,
      answerAction,
      count: subjects.length,
      // What they actually wrote, which is the most useful thing here by far.
      answers: rows.map((r) => r.answerText).filter(Boolean).slice(0, 5),
      applied: rows.reduce((n, r) => n + (Number(r.timesApplied) || 0), 0),
...
/**
 * What to say about it — and what NOT to say.
 *
 * A REPEATED "NO" DOES NOT PROPOSE AUTOPILOT. The owner saying "leave this
 * alone" three times is evidence the check should ask less, not that it should
 * act more, and `disable_auto_apply` is the only direction this registry moves
 * in anyway. A repeated "YES" is the one that looks like a case for autopilot —
 * and it is deliberately left as WORDS with no action, because a machine
 * proposing that it be trusted with more is the exact shape nobody should
 * build, however many confirmations sit in front of it. The owner turns
 * autopilot on from the Automation screen, where the permission is visible.
 */
function describeOwnerAnswerGroup(group) {
  const what = group.checkKey.replace(/[._]/g, ' ');
  const said = group.answers.length
    ? ` They said: ${group.answers.slice(0, 3).map((a) => `"${a}"`).join('; ')}.`
    : '';
  if (group.answerAction === 'dismiss') {
    return {
      kind: 'repeated_owner_answer',
      subjectId: `${group.checkKey}|dismiss`,
      title: `You have told Wenze "no" about "${what}" ${group.count} times`,
      lines: [
        `${group.count} different cases in the last month, all answered the same way.`,
        group.applied
          ? `Those answers have since closed ${group.applied} further case${group.applied === 1 ? '' : 's'} without asking you.`
          : null,
      ].filter(Boolean),
      suggestion: 'This check may be looking for something that is not a problem here. '
        + 'Consider switching its automatic correction OFF so it stops proposing, or '
        + `telling us what it should be looking for instead.${said}`,
      evidence: {
        checkKey: group.checkKey, answerAction: group.answerAction,
        count: group.count, answers: group.answers, subjects: group.subjects,
      },
      applyAction: { action: 'disable_auto_apply', payload: { checkKeys: [group.checkKey] } },
    };
  }
  return {
    kind: 'repeated_owner_answer',
    subjectId: `${group.checkKey}|approve`,
    title: `You have said "yes" to "${what}" ${group.count} times`,
    lines: [
      `${group.count} different cases in the last month, all approved.`,
      'Wenze has not applied any of them by itself, and will not: an answer about '
        + 'one case is not a standing permission.',
    ],
    suggestion: 'If this is always the right answer, Automation is where you can let '
      + 'Wenze do it without asking. That switch is yours — nothing here changes it.',
    evidence: {
      checkKey: group.checkKey, answerAction: group.answerAction,
      count: group.count, subjects: group.subjects,
    },
    // NULL, ON PURPOSE. There is no `enable_auto_apply` and there will not be.
    applyAction: null,
  };
}

/**
 * A concrete threshold to agree to, instead of a paragraph about judgement.
 *
 * `describeContradictionGroup` above already says a check keeps being wrong, and
 * its advice about the confidence floor is deliberately words — choosing how
 * cautious a business wants to be is not a machine's call. That stays true. What
 * changes here is that the NUMBER is no longer a matter of taste: every decision
 * carries the confidence it acted at and whether it later held, so the lowest
 * floor at which this check's own record is acceptable is a measurement.
 *
 * `lib/operations/thresholdProposal.js` does the arithmetic. This turns it into
 * the four lines an administrator reads — current, suggested, evidence, effect —
 * and attaches the action only when the change is one Wenze is allowed to make.
 *
 * A PROPOSAL TO LOOSEN CARRIES NO ACTION, on purpose, and says so in the text:
 * accepting it means "yes, and I will go and change it". Pretending otherwise
 * is the failure this whole layer was built to end.
 */
function describeThresholdProposal(proposal) {
  const what = proposal.checkKey.replace(/[._]/g, ' ');
  const inherited = proposal.currentFloorIsInherited ? ' (inherited default)' : '';

  if (proposal.direction === 'lower') {
    return {
      kind: 'threshold_proposal',
      subjectId: proposal.checkKey,
      title: `"${what}" may be refusing more than it needs to`,
      lines: [
        `Current confidence threshold: ${proposal.currentFloor}`,
        'Suggested threshold: lower — Wenze will not name a number for this',
        `Evidence: ${proposal.evidence.heldBelowFloor} decisions were held below the floor, `
          + `and ${proposal.evidence.rateBelowFloor}% of comparable ones held up.`,
        `Expected effect: ${proposal.expectedEffect}`,
      ],
      suggestion: 'Consider lowering this check\'s confidence floor in Settings → AI & Autonomy. '
        + 'Accepting this records your agreement; the change itself is yours to make.',
      evidence: { ...proposal.evidence, direction: 'lower', applicable: false },
      // NO ACTION. Loosening a safety margin is never Wenze's to apply.
      applyAction: null,
    };
  }
  ...
  return {
    kind: 'threshold_proposal',
    subjectId: proposal.checkKey,
    title: `"${what}" acts at ${proposal.currentFloor} and holds up ${proposal.evidence.rateAtCurrent}% of the time`,
    lines: [
      `Current confidence threshold: ${proposal.currentFloor}${inherited}`,
      `Suggested threshold: ${proposal.suggestedFloor}`,
      `Evidence: of ${proposal.evidence.graded} graded decisions, `
        + `${proposal.evidence.rateAtCurrent}% held up at the current floor and `
        + `${proposal.evidence.rateAtSuggested}% held up at or above ${proposal.suggestedFloor}.`,
      `Expected effect: ${proposal.expectedEffect}`,
    ],
    suggestion: `Raise this check's confidence floor to ${proposal.suggestedFloor}. It will keep `
      + 'finding the problem and wait for a person when it is less sure.',
    evidence: { ...proposal.evidence, direction: 'raise', applicable: true },
    applyAction: {
      action: 'raise_confidence_floor',
      payload: {
        checkKey: proposal.checkKey,
        currentFloor: proposal.currentFloor,
        suggestedFloor: proposal.suggestedFloor,
      },
    },
  };
}

/** Every check with enough graded history to say something concrete about. */
function proposeThresholds(outcomes, floors = {}, opts = {}) {
  const byCheck = outcomes?.byCheck || {};
  const out = [];
  for (const [checkKey, graded] of Object.entries(byCheck)) {
    const proposal = proposeConfidenceFloor({
      checkKey, graded, currentFloor: floors[checkKey] ?? null, options: opts,
    });
    if (proposal) out.push(describeThresholdProposal(proposal));
  }
  return out;
}
...
function findLessons(sources = {}, options = {}) {
  ...
  const suggestions = [
    ...
    ...groupOwnerAnswers(sources.memories, { nowMs, ...opts }).map(describeOwnerAnswerGroup),
    // Optional by construction: a caller that has not wired the outcome read
    // loses THESE suggestions, never the ones above it.
    ...proposeThresholds(sources.confidenceOutcomes, sources.checkFloors, options.threshold),
  ];
...
module.exports = {
  ...
  groupOwnerAnswers,
  describeOwnerAnswerGroup,
  describeThresholdProposal,
  proposeThresholds,
  findLessons,
};
