 ...
 * been out five weeks and the home window they were promised did not happen,
 * and the actionable half of that sentence is entirely the company's.
 *
...
function assess(driver, options = {}) {
  ...
  // `home_request_unanswered` WAS HERE, and it was the loudest wrong thing this
  // file said. A home-time request is recorded and three managers are told; it
  // is not a ticket anybody answers, so a request whose dates went by is not
  // the company ignoring a driver — it is a calendar. Scoring it put drivers on
  // the call list for nothing, and the fleet-wide count it produced ("34
  // drivers worth a call") was mostly this. A home window actually AGREED and
  // then not honoured is still scored, above, because that is a promise.
...
function suggestActions(signals) {
  ...
  if (by.has('home_commitment_broken')) {
    actions.push('Give them a home date they can hold you to — the last one did not happen');
  }
