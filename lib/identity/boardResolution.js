...
/**
 * A TEAM TRUCK HAS TWO PEOPLE IN IT, AND THAT IS NOT AN AMBIGUITY.
 *
 * `decideBoardLink` refuses outright when a truck holds more than one
 * same-fleet person, and for a single driver it is right to: two people
 * recorded in one truck means one of the records is wrong, and picking either
 * is a guess with somebody's bonus attached. For a TEAM row the same fact is
 * the expected shape — seats 1 and 2 — and the caller knows something the
 * single-driver path never has: WHICH member it is asking about.
 *
 * So the ambiguity is re-examined with that name, and only settled when the
 * evidence is exactly as strong as the auto-link rule demands elsewhere: the
 * truck matched on its real spelling, and the member's name agrees STRICTLY
 * with exactly one of the people in it. Two independent exact matches are not
 * a looser standard than one; they are the same standard, applied twice.
 *
 * Anything short of that stays as `decideBoardLink` left it — a question.
 * Without this the two members reduce to "more than one person is recorded in
 * this truck", and every team driver falls off every roster.
 */
function settleAgainstSeats(name, decision, unitHolders, row) {
  if (decision.action !== 'ask' || decision.checkKey !== CHECKS.AMBIGUOUS) return decision;
  const holders = (Array.isArray(unitHolders) ? unitHolders : [])
    .filter((h) => sameFleet(row.fleetType, h.fleetType));
  if (holders.length < 2) return decision;
  // A digits-only truck match may never act, here as anywhere else.
  if (holders.some((h) => h.truckExact === false)) return decision;
  const named = holders.filter((h) => namesAgreeStrictly(name, h.displayName));
  if (named.length !== 1) return decision;
  return {
    action: 'link',
    personId: named[0].personId,
    confidence: LINK_CONFIDENCE,
    checkKey: null,
    reason: 'this member of the team holds the truck and the name agrees',
  };
}
...
function decideTeamBoardLink({ row = {}, members = [], lookupFor = () => ({}) } = {}) {
  const decided = members.map((member) => {
    const lookup = lookupFor(member) || {};
    const decision = decideBoardLink({ row, name: member, ...lookup });
    return { name: member, ...settleAgainstSeats(member, decision, lookup.unitHolders, row) };
  });
