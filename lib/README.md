...
| `security/redactUrls.js` | Takes every URL and stray credential pair out of a sentence before it is logged, stored or returned. The Dispatcher Board authenticates by query string, so an error message that quotes the request is a leaked token. `splitCredentialsFromUrl` beside it takes the credential OUT of a pasted URL before it can be stored in plaintext, handing back a `token` for the caller to encrypt and silently dropping any other credential-shaped parameter. |
...
| `database/queryLabel.js` | Which table a statement names, for the transfer meter's per-table breakdown. Runs on EVERY query, so it is one regular expression and no parser — and it returns an identifier or `other`, **never a fragment of the SQL**, because a label that echoed part of a WHERE clause would put driver names and money codes in a diagnostics endpoint. Comments and quoted literals are blanked before the match: the capture shape alone is not enough, since `alice_smith` out of a comment is a well-formed identifier and a person's name. |
