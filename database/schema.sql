...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'trailers_master_status_check') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'trailers_master_source_check') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'trailers_merged_target_check') THEN
