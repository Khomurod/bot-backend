...
- **The Node suite passes clean with no secrets and no database.** Verified
  ...
  local PostgreSQL 16): **4036 tests, 4036 pass, 0 fail, 0 skipped**, exit 0.
  Split the way CI splits it: the 330 non-`*Pg` files with no application env at
  all are **3487 pass / 0 skipped**, and the 60 `*Pg` files against a real
  Postgres are **549 pass / 0 skipped**. The admin suite is **232 pass in 26
  files**.
