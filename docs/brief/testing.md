...
- **The Node suite passes clean with no secrets and no database.** Verified
  ...
  local PostgreSQL 16): **4624 tests, 4624 pass, 0 fail, 0 skipped**, exit 0.
  Split the way CI splits it: the 359 non-`*Pg` files with no application env at
  all are **3964 pass / 0 skipped**, and the 72 `*Pg` files against a real
  Postgres are **660 pass / 0 skipped**. The admin suite is **346 pass in 36
