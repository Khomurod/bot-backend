...
- **The Node suite passes clean with no secrets and no database.** Verified
  ...
  local PostgreSQL 16): **3877 tests, 3877 pass, 0 fail, 0 skipped**, exit 0.
  Split the way CI splits it: the 322 non-`*Pg` files with no application env at
  all are **3363 pass / 0 skipped**, and the 58 `*Pg` files against a real
  Postgres are **514 pass / 0 skipped**. The admin suite is **224 pass in 25
  files**.
