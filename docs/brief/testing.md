...
- **The Node suite passes clean with no secrets and no database.** Verified
  ...
  local PostgreSQL 16): **4633 tests, 4633 pass, 0 fail, 0 skipped**, exit 0.
  Split the way CI splits it: the 359 non-`*Pg` files with no application env at
  all are **3969 pass / 0 skipped**, and the 73 `*Pg` files against a real
  Postgres are **664 pass / 0 skipped**. The admin suite is **346 pass in 36
