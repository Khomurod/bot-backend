...
- **The Node suite passes clean with no secrets and no database.** Verified
  ...
  local PostgreSQL 16): **4082 tests, 4082 pass, 0 fail, 0 skipped**, exit 0.
  Split the way CI splits it: the 333 non-`*Pg` files with no application env at
  all are **3526 pass / 0 skipped**, and the 61 `*Pg` files against a real
  Postgres are **556 pass / 0 skipped**. The admin suite is **234 pass in 26
  files**.
