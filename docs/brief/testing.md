...
- **The Node suite passes clean with no secrets and no database.** Verified
  ...
  local PostgreSQL 16): **4747 tests, 4747 pass, 0 fail, 0 skipped**, exit 0.
  Split the way CI splits it: the 364 non-`*Pg` files with no application env at
  all are **4061 pass / 0 skipped**, and the 75 `*Pg` files against a real
  Postgres are **686 pass / 0 skipped**. The admin suite is **353 pass in 37
