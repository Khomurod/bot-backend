...
| **Dispatcher Board** (external Google Apps Script over the dispatchers' own sheet, read-only) | Settings → Dispatcher Board (`dispatch_board_settings`, single row, token encrypted). **No env fallback and `enabled` defaults FALSE** — there is nothing to inherit and an integration that switches itself on at deploy starts making requests nobody reviewed | the authority on today's assignment: truck, trailer, status, ETA, dispatcher, team | read every 5 minutes (60–3600s, configurable) by `services/dispatchBoard/poller.js` into `dispatch_board_rows` — the only writer of that snapshot — and by nothing else. Nothing is read until a URL and token are saved. **The token travels in the query string**, so every error leaves through `lib/security/redactUrls.stripUrls` and `last_error` can never hold a URL; exactly one redirect is followed and the token is not re-attached to it. See [`docs/architecture/dispatcher-board.md`](../architecture/dispatcher-board.md) |
| **Finance Monitor** (one Telegram group, read-only) | Settings → Finance Monitor (`finance_settings`, single row). `chat_id` NULL inherits `FINANCE_GROUP_CHAT_ID`; **`enabled` inherits nothing and defaults FALSE**, and cannot be set until the group has been validated — an env var must never be able to start reading a payment chat | the record of money codes issued in that group, so "did we already send that one?" is not answered by scrolling | nothing is read from any chat until it is switched on. Capture is an observer: it never consumes a message and never throws at the bot, and it QUEUES attachments rather than fetching them — downloading inside Telegram's message pipeline would hold every driver's message behind it. A background reader takes them one at a time. A capture failure is logged as ids and statuses — never the text, and never the download URL, which embeds the bot token. See [`docs/architecture/finance-monitor.md`](../architecture/finance-monitor.md) |
