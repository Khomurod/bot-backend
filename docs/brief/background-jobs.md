...
| `control/askPass` | rides the consistency timer (15 min), after the contradiction pass and before the notification drain | turns open findings the owner has not yet permitted into ONE plain question each in the notifications group, capped at `max_questions_per_pass` (5) per pass AND as a standing cap on questions still unanswered — so it asks nothing while the owner is still working through the last five — oldest first, not repeated inside `repeat_after_hours` (72). Also the caller that finally writes `suggest` rows to the decision journal. Asks nothing when the channel is switched off |
