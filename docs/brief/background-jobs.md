...
| `control/askPass` | rides the consistency timer (15 min), after the contradiction pass and before the notification drain | turns open findings the owner has not yet permitted into ONE plain question each in the notifications group, capped at `max_questions_per_pass` (5) per pass AND as a standing cap on questions still unanswered — so it asks nothing while the owner is still working through the last five — oldest first, not repeated inside `repeat_after_hours` (72). Also the caller that finally writes `suggest` rows to the decision journal. Asks nothing when the channel is switched off. Before asking, it checks `control_knowledge`: a finding whose condition the owner has already answered "no" to is closed from that answer instead of asked again, and `times_applied` is counted. Also asks about findings whose check IS on autopilot but whose latest decision was `hold` or `unknown` (`operationalDecisions.currentHolds`), with the reason in plain words — before this, granting autonomy could produce silence |
| `routeControlService` (monitor) | settings-driven, floor 30s | destination completion + off-route warnings |
| `finance/weeklyReportService` | sleeps until Monday 08:00 America/Chicago, capped at an hour | the week's money-code summary, **once per period and never twice**. The claim in `service_runs` is taken BEFORE any work, so a redeploy on a Monday morning cannot send it again; a failed send RELEASES the claim, so it retries rather than being lost forever. One week per tick, never a loop over missed weeks. A period that began before the monitor was switched on is recorded `suppressed_backfill` and not sent — "$0 issued" for a week nobody watched is the one wrong answer. `blocked` while it is switched off |
| `finance/documentReader` | event wake from capture, an exact retry wake after each drain, 15 min idle sweep | reads the attachments posted in the finance group **one at a time** — a document is held whole in memory while it is read, so the claim takes exactly one row (`FOR UPDATE SKIP LOCKED … LIMIT 1`) and a drain stops after 5. Attempts count when a row is TAKEN, not when it fails, so a crash loop stays bounded; a claim abandoned by a crash is released after 15 min with its attempt already spent. **`failed` is "could not fetch" and retries (5→15→30→60→60 min, then stops); `needs_review` is "could not read well enough" and never retries — a person is what it needs.** AI being unavailable is `needs_review`, never `failed`. `blocked` while the Finance Monitor or document capture is off |
| `finance/reparsePass` | rides the scheduler's retention tick — **at boot**, then hourly | re-reads captured messages an older parser version misunderstood, oldest first, 25 at a time, through the same path the Re-read button uses. A version bump is exactly what creates this backlog, and firing at boot is exactly when it appears. Safe to repeat: `ON CONFLICT (message_ref_id, code_normalized) DO NOTHING` records no second code and a re-read never resurrects a voided one. A re-read that newly recognises a VOID resolves it rather than only re-labelling the message. It then offers up to 5 still-unclear messages to a model — **once each**, stamped only when something answered, so a provider outage does not spend a message's one chance — and an AI-read void marks the code as needing a person, never voids it. `blocked` when the Finance Monitor tables are not there |
