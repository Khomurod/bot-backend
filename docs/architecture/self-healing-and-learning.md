...
Five production incidents taught this system that a component can be configured,
answer `ok`, and be completely inert — and that the report itself can freeze, or
silently stop covering a component, while still reading healthy. The rules, and
the reading behind each, are in
**[`health-reporting.md`](health-reporting.md)**: switched off is not broken; a
custom integration must be able to say `blocked`; the watch must supply every
dependency its own observations need; a pass that observed nothing is a failed
pass; and configured is not the same as reachable.
