...
| **The control channel** — answering Wenze in Telegram | pure: `lib/control/{intent,askable}.js`; data: `database/{controlSettings,controlOperators,controlReplies}.js`; asking: `services/control/askPass.js` (on the consistency timer); answering: `bot/controlReplyHandlers.js` → `services/control/replyHandler.js` → `services/control/actions.js` (the ONLY writer); admin: `server/routes/settings/controlRoutes.js`, `admin/src/pages/settings/control/ControlChannelCard.jsx` |
