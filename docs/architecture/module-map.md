...
| **Pure helpers / constants** — the bottom layer | `lib/` — modules with no I/O and no mutable state, in domain subdirectories: `lib/security/facebookCrypto.js`, `lib/rbac/roleKeys.js`, `lib/drivers/{driverGroupTitle,driverProfileParse}.js`, `lib/telegram/telegramUsername.js`, `lib/routeControl/routeControlConstants.js`, `lib/geo/distance.js`, `lib/database/{failureClassification,queryLabel}.js`, `lib/decisions/{rollbackSafety,sources,confidence}.js`, `lib/operations/{runHealth,healthTransitions,backgroundServiceCatalog,learning,thresholdProposal}.js`, `lib/recruiting/{workingHours,quietHours,reachability,readiness,replyGuard,thread}.js`. They used to live in `services/`, a layer ABOVE the database, which forced nineteen `database/**` modules to depend upward; `database/**` now depends on nothing above it. See [`lib/README.md`](../../lib/README.md) for the charter and the rule for adding to it. |
...
| `services/operations/healthObservations.js` | `services/operations/observations/{shape,integrations}.js` — the façade keeps the ledger-driven worker half and `gatherAllObservations`; `shape.js` owns the one shape every observation takes, so a component cannot answer two different things in one payload |
