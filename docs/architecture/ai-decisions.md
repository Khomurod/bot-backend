...
| **Reading an answer you typed in the chat** (`control_reply_reading`) | Whatever the finding's own answer writes — an applied correction, a dismissal, a delay | Reached ONLY after `lib/control/intent.js` returns `unclear`, so nearly every reply is decided with no model at all. It may pick only from the keys that question already offered, checked as the router's validator and again on the finished object; it supplies no values, so nothing it returns can land in a driver's record. Unavailable, malformed or unoffered all mean `unclear` — the question stands | **Automatic for the reading, never for the permission.** An approval still goes through `applyCorrection` at approval tier with the operator as initiator, and a remembered approval is never re-applied |
| **Reading a finance document** (`finance_document_extraction`) | That one document's own `finance_documents.extracted` row, and nothing else | It is asked for what is PRINTED — never for a judgement, a total or whether something looks wrong. The document's text and caption are fenced as untrusted data, the answer must pass a schema validator (the router treats a failure as a provider failure and moves on), and only the declared keys are kept, so an invented one cannot ride into a JSONB column. No total, no report figure and no duplicate decision ever comes from it — those are counted in SQL from the captured text. A low confidence, a missing amount or date, an unavailable provider: all `needs_review`, never a stored fact | **Transcription only.** The money code that counts is the one read deterministically out of the message text by `lib/finance/moneycode.js`; a document reading is evidence beside it, and a person is asked whenever it is not certain |
| **Saying what a finance message MEANS** (`finance_message_reading`) | Nothing directly. It proposes a READING of a message the rules could not settle — a kind, and a pointer to a code already in the text | It is asked for MEANING, never for measurement: is this issuing a code, reporting a void, asking for one, or ordinary conversation. It is reached only when the deterministic parser returned `unparsed` or `ambiguous`, so a message the rules read cleanly never costs a call. Every number it returns goes through `lib/finance/aiReading.js`, which looks for that exact run of digits in the captured text and drops what it cannot find — a substring of a real code is refused too, because it is a different number. A reading that claims a code the message does not contain is refused whole. It never decides WHICH code a void refers to; `lib/finance/void/target.js` does that from reply relationships and refuses to choose between two candidates | **Interpretation only.** A model can move a message off the unclear pile. It cannot author a digit, an amount or a void — and with no provider at all the Finance Monitor behaves exactly as it does today |
